                ...
                <div class="card-ku h-100 rounded-3 overflow-hidden"
                    ...
                    @if($event->image)
                        <img src="{{ Storage::url($event->image) }}"
                             class="w-100" style="height:180px; object-fit:cover" alt="{{ $event->title }}">
                    @else
                    ...
                    <div class="p-3">
                        <span class="badge mb-2" style="background:var(--ku-accent); color:#111">
                            <i class="bi bi-clock me-1"></i>{{ $event->event_date->format('M j, Y') }}
                        </span>
                        <h5 class="fw-bold mb-0" style="color:var(--ku-accent)">{{ $event->title }}</h5>
